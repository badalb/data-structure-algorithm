...
public class PrintSortedMatrixInLinearForm {
	public static void main(String args[]) throws Exception {
		int[][] matrix = { { 5, 23, 30, 40 }, { 14, 25, 34, 41 }, { 28, 30, 37, 48 }, { 32, 33, 40, 54 }, };
		System.out.println(Arrays.toString(convert(matrix)));
	}

	private static int[] convert(int[][] matrix) throws Exception {
		int[] sortedArr = new int[matrix.length * matrix[0].length];

		MinHeap minHeap = new MinHeap(matrix.length);
		int k = 0;
